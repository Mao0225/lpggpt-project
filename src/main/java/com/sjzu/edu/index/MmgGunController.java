...
public class MmgGunController extends Controller {
    MmgGunService MgGunService = new MmgGunService();
    ...
    public void gunlist(){
        ...
        Page<Record> BseGunPage=MgGunService.paginate(pageNumber,pageSize,stationid);
    ...
    public void searchgun(){
        ...
        int pageNumber = getParaToInt("page", 1); // 默认为第1页
        int pageSize = getParaToInt("size", 10);
        Page<Record> BseGunPage=MgGunService.search(pageNumber,pageSize,stationid);
        List<GasStation> gastations = gastation.find("SELECT * FROM gas_station");
        setAttr("gastations", gastations);
        setAttr("gunlist", BseGunPage);
    ...
    public void add(){
        List<GasStation> gastations = gastation.find("SELECT * FROM gas_station WHERE id ="+ getSessionAttr("stationid") +"");
        setAttr("gstations", gastations);
        render("add.html");
    }
    public void save() {
