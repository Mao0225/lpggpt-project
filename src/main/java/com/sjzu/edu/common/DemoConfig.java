 ...
 * 详见 JFinal 俱乐 https://jfinal.com/club
 * 
 * API 引导式配
 */
public class DemoConfig extends JFinalConfig {
	...
	/**
	 * 启动入口，运行此 main 方法可以启动项目，此 main 方法可以放置在任意的 Class 类定义中，不一定要放于
	 */
	...
	/**
	 * PropKit.useFirstFound(...) 使用参数中从左到右最先被找到的配置文
	...
	public void configRoute(Routes me) {
		...
		me.add("/applpgyunxing", ApplpgyunxingController.class);//这个主要是给大屏幕用
		me.add("/appbangding", AppbangdingController.class);//这个是绑定称应用程序
		me.add("/appranqibiao", AppranqibiaoController.class);//这个是给丹东燃气表做的接受燃气表上传信息
		me.add("/shexiangtou", AppshexiangtouController.class);//这个是管理摄像头信息的，首先是收火警
		me.add("/appranqibiaoanzhuang", AppranqibiaoanzhuangController.class);//这个是给丹东燃气表做的接受燃气表上传信息
		me.add("/appipaddress", AppipaddressController.class);//这个是给丹东燃气表做的接受燃气表上传信息
		...
		me.add("/apprecord",AppRecordController.class);//LPG手机端充装追溯记
        ...
        me.add("/gasDocs",GasDocsController.class);
		//测试连接Github
	}
